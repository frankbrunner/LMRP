...
def moveAlongBoundary():
     ...
     turnDirection = getdirectionToTurn(directionToWaypoint,bearing)
     """turn in direction of first waypoint"""
     turnRobot(turnDirection,directionToWaypoint)
     """set global Variable for GPS DATA Loop to start moving"""
     #nextWP = gpsWayPoints[index]
     #directionToNextWp = directionToWaypoint
     menue = "main"

def turnRobot(turnDirection,directionToWP):
     deviation = 5
     while True:
          if int(bearing) <= int(directionToWP) + int(deviation) and int(bearing) >= int(directionToWP) - int(deviation):
               move.stop()
               break
          else:
